...
class HybridMarchingSquaresMPI {
private:
    int rank, size;
    MPI_Comm cart_comm;
    int dims[2], coords[2];
...
public:
    HybridMarchingSquaresMPI(double xmin, double xmax, double ymin, double ymax, int M, int N)
        ...
        // Obtener número de threads OpenMP
        #pragma omp parallel
        {
            #pragma omp single
        ...
        if (rank == 0) {
            std::cout << "Configuración híbrida: " << size << " procesos MPI × " 
                      << num_threads << " threads OpenMP\n";
            std::cout << "Topología cartesiana: " << dims[0] << "×" << dims[1] << "\n";
        }
    }

    ~HybridMarchingSquaresMPI() {
    ...
    void process_all_local_cells() {
        ...
        std::vector<std::vector<LineSegment>> thread_segments(num_threads);
        ...
        #pragma omp parallel
        {
            ...
            for (int j = 0; j < local_N; ++j) {
                for (int i = 0; i < local_M; ++i) {
                    process_cell_independent(i, j, thread_segments[tid]);
                }
            }
        }
        
        for (const auto& segments : thread_segments) {
            all_local_segments.insert(all_local_segments.end(), 
                                    segments.begin(), segments.end());
        }
    }

    // Misma función de escritura que tu código original
    ...
    void run() {
        ...
        write_eps_parallel("output_hybrid.eps");
        ...
        if (rank == 0) {
            ...
            std::cout << "--- Resultados Híbridos MPI+OpenMP ---\n";
            std::cout << "Grid: " << global_M << "x" << global_N << "\n";
            std::cout << "Procesos MPI: " << size << " (" << dims[0] << "x" << dims[1] << ")\n";
            std::cout << "Threads OpenMP por proceso: " << num_threads << "\n";
            std::cout << "Total de unidades de cómputo: " << size * num_threads << "\n";
            ...
            std::cout << "Salida guardada en: output_hybrid.eps\n";
        }
    }
};
...
int main(int argc, char* argv[]) {
    ...
    if (provided < MPI_THREAD_FUNNELED) {
        std::cerr << "Error: MPI no soporta el nivel de threads requerido\n";
...

    {
        ...
        HybridMarchingSquaresMPI ms(x_min, x_max, y_min, y_max, M, N);
        ms.run();
    }
